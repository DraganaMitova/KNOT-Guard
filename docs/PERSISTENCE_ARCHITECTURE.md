...
The PostgreSQL adapter has been validated by an internal integration proof on the `integration-hardening-pass` branch. The proof starts a temporary embedded PostgreSQL server by default. It can also run against an external test database with `KNOT_POSTGRES_TEST_URL`.
...
The proof creates isolated audit and token tables, runs concurrent executions against one token, asserts that exactly one execution wins, checks replay rejection for the rest, and verifies the resulting audit chain.
